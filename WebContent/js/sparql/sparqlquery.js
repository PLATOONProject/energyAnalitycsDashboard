...
var sessionGraph = sessionStorage.getItem("graph");
...
//	$.getJSON(queryUrlEncoded, callbackFunction).error(function() { alert("There was an error during communication with the sparql endpoint"); });
	...
	$.ajax({
	    ...
	    error: function() { alert("There was an error during communication with the sparql endpoint-execSparqlTopGeoBroaderNarrower");}
	});
}

function execSparqlAllGeoCodes(callbackFunction) {//FIND ALL ELEMENTS
...
//	$.getJSON(queryUrlEncoded, callbackFunction).error(function() { alert("There was an error during communication with the sparql endpoint"); });
	...
	$.ajax({
		...
		dataType: 'jsonp',
	    success: callbackFunction,
	    error: function() { alert("There was an error during communication with the sparql endpoint-execSparqlBroaderNarrowerForArray");}
	});
}
